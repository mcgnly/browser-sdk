window.RELAYR = (function(win, doc) {
    ...
    function Relayr(credentials) {
        ...
        function devices() {
            ...
            this.update = function(options, successCb, errorCb) {
                if (!this.options) {
            ...
            this.sendCommand = function(device, successCb, errorCb) {
                ...
                util.ajax({
                    ...
                    token: "Bearer " + device.token,
                    body: device.command
                }, function(device) {successCb(device);
                }, function(error) {
                    errorCb(error);
                    // localStorage.removeItem("relayrToken");
                    // self.login();
        ...
        function transmitters() {
            ...
            this.delete = function(successCb, errorCb) {
                util.ajax({
                    url: self.config.url + "transmitters/" + self.transmitterId,
                    type: "DELETE",
                    token: "Bearer " + self.account.token,
                    isObject: true
